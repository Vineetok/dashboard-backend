...
export const getProfile = async (req, res) => {
  try {
    ...
    const query = `
      SELECT id, adv_id, name, email, mobile, pan, pan_verified, email_verified, state, city, head, category, date_joined , entity_type, referral_code
...
export const getKycStatus = async (req, res) => {
  try {
    ...
    const kycStatus = `SELECT kyc_completed FROM tbl_dsa_kyc WHERE user_id = $1`;

    const { rows } = await pool.query(kycStatus, [userId]);
    ...
    return res.status(200).json({
      ...
      kycStatus: rows[0].kyc_completed,
    });
  } catch (err) {
...
export const updateProfile = async (req, res) => {
  try {
    const userId = req.user?.id;
    const { email, current_address, head, category, password } = req.body;
    ...
    const kycUpdateQuery = `UPDATE tbl_dsa_kyc SET current_address = $1 WHERE user_id = $2 RETURNING current_address`;

    const { rows: currentAddress } = await pool.query(kycUpdateQuery, [current_address, userId]);
    ...
    return res.status(200).json({
      ...
      user: rows[0],
      current_address: currentAddress[0]?.current_address || null,
    });
  } catch (err) {
  ...
  try {
    ...
    await resend.emails.send({
      ...
      <div style="font-family: Arial, sans-serif; background:#f5f7fb; padding:30px;">
        <div style="max-width:700px;margin:auto;background:white;border-radius:10px;overflow:hidden;box-shadow:0 5px 20px rgba(0,0,0,0.08);">
...
export const verifyEmailOtp = async (req, res) => {
  try {
    ...
    await updateKYCCompletion(userId);
...
export const createReferralLead = async (req, res) => {
  ...
  `,
    ...
    //   userId: lead.rm_id, // RM who owns the DSA
    //   title: "New Lead Assigned",
    //   body: `A new lead ${lead.lead_name} has been created by DSA`,
    //   type: "lead_created"
    // });
...
export const createDetailLead = async (req, res) => {
  try {
    ...
    SELECT id, detail_lead_id
    FROM tbl_detail_leads
    WHERE contact_number = $1
      AND (
        email = $2 OR $2 IS NULL
      )
      AND LOWER(sub_category) = LOWER($3)
      AND lead_status != 'REJECTED'
    LIMIT 1
    `,
      [client.mobile, client.email || null, sub_category],
    );

    if (duplicateCheck.rows.length > 0) {
      return res.status(409).json({
        success: false,
        message: "This lead already exists for this client",
        existing_lead_id: duplicateCheck.rows[0].detail_lead_id,
      });
    }
...
export const refreshToken = async (req, res) => {
  try {
    ...
    const appVersions = await pool.query("select * from tbl_app_versions");

    return res.status(200).json({
      message: "Token refreshed successfully.",
      token: newToken,
      version: '1.0.0',
      android_version: appVersions.rows[0].android_app_version,
      ios_version: appVersions.rows[0].ios_app_version,
    });
...
export const generateAadhaarOtp = async (req, res) => {
  try {
    ...
    //   `SELECT pan_verified FROM tbl_registeredusers WHERE id = $1`,
    //   [userId]
    // );

    // if (!user[0]?.pan_verified) {
    //   return res.status(400).json({
    //     message: "Verify PAN first"
    //   });
    // }
...
export const verifyPanAadhaarLink = async (req, res) => {
  try {
    ...
    if (!pan || pan === "NA" || pan_verified === false) {
      return res.status(400).json({
...
// 🔹 Helper function to check if all KYC steps are complete and set kyc_completed flag
const updateKYCCompletion = async (userId) => {
  await pool.query(`
    UPDATE tbl_dsa_kyc k
    SET 
      kyc_completed = (
        SELECT 
          COALESCE(u.email_verified, false)
          AND COALESCE(k.phone_verified, false)
          AND COALESCE(u.pan_verified, false)
          AND COALESCE(k.aadhaar_verified, false)
          AND COALESCE(k.pan_aadhaar_linked, false)
          AND COALESCE(k.bank_verified, false)
          AND (
            -- GST rule
            u.entity_type = 'Individual'
            OR COALESCE(k.gst_verified, false)
          )
    ...
    WHERE k.user_id = $1
      ...
      AND k.kyc_completed IS DISTINCT FROM (
        SELECT 
          COALESCE(u.email_verified, false)
          AND COALESCE(k.phone_verified, false)
          AND COALESCE(u.pan_verified, false)
...
// 🔹 GET USER TDS (For Download / View)
export const getUserTds = async (req, res) => {
  try {
    ...
    const tdsTotal = `
    SELECT SUM(tds_amount) FROM tbl_detail_leads 
    where dsa_id = $1 AND lead_status = 'COMPLETED' 
    `
    const { rows: tdsTotalRows } = await pool.query(tdsTotal, [userId]);

    const { rows } = await pool.query(query, [userId]);

    if (rows.length === 0) {
      return res.status(404).json({
        message: "TDS data not found."
      });
    }

    return res.status(200).json({
      message: "TDS data fetched successfully.",
      tds: rows[0],
      tdsTotal: tdsTotalRows[0]
    });

  } catch (error) {
    console.error("Get User TDS Error:", error);
...
// Create Agreement (CaseDock Integration)
export const createAgreement = async (req, res) => {
  try {
    ...
    // 🔹 1. Fetch user
    const userQuery = `
      SELECT id, name, email, mobile, pan, entity_type
      FROM tbl_registeredusers
      WHERE id = $1
      LIMIT 1;
    `;
    ...
    const { rows: kycRows } = await pool.query(kycQuery, [userId]);
    ...
    if (!kyc || !kyc.kyc_completed) {
      return res.status(400).json({
        message: "KYC not completed. Agreement cannot be created.",
      });
    }
    ...
    // 🔹 4. Extract Aadhaar Data
    const aadhaarData = kyc.aadhaar_kyc_data || {};
    const address = aadhaarData.address || {};

    if (!aadhaarData.full_address && !address) {
      return res.status(400).json({
        message: "Aadhaar address not available.",
      });
    }

    // ✅ Full address (best approach)
    const fullAddress =
    ...
    // 🔹 5. Calculate Age
    let age = "";
    if (aadhaarData.date_of_birth) {
      const [day, month, year] = aadhaarData.date_of_birth.split("-");
    ...
    const middleName = nameParts[1] || "NA";
    const lastName = nameParts[2] || "NA";

    // 🔹 7. Build Payload
    const payload = {
      request_book_uuid: process.env.CASE_DOCKER_UUID,
      name: `${user.name}`,
      initiator_email: "admin@infinityarthvishva.com",
      data: {
        party_data: {
          email: user.email,
          tax_id: "",
          ...
          is_vendor: false,
          last_name: lastName,
          partyType: user.entity_type || "Individual",
        ...
        party_pan: user.pan || "",
        party_aadhaar: kyc.aadhaar_number || "",
        party_age: age ? String(age) : "",
      },

      send_for_external_filling: true,
    ...
    // 🔹 8. Call External API
    const response = await axios.post(
      `${process.env.CASE_DOCKER_URL}/external/playbook/requests/`,
      payload,
      {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${process.env.CASE_DOCKER_API_KEY}`,
        },
      }
    );

    // 🔹 9. Save Agreement Record
    await pool.query(
      `INSERT INTO tbl_dsa_agreements (user_id, external_uuid, status)
       VALUES ($1, $2, $3)`,
      [userId, response.data?.uuid || null, "created"]
    );
    ...
    console.error("Agreement Error:", error?.response?.data || error.message);
